(function () {
    return function (parameters, TagManager) {
        ...
        var advertiser_id = parameters.get('advertiser_id');
        this.fire = function () {
            if (!isLibLoaded) {
                isLibLoaded = true;
                var utm268 = {};
                utm268.data = {
                    /* Settings General */
                    window['advertiser_id'] = parameters.get('advertiser_id'), // do not change
                    window['pt_pagetype'] = parameters.get('pt_pagetype'), // insert pagetype, possible values: 'home', 'product', 'category', 'conversion'
                    window['pt_checkout'] = parameters.get('pt_checkout'), // insert orderstep 1-n (optional)
                    /* Conversion */
                    ...
                    /* Do not change */
                    window['pt_loc'] = decodeURI(document.location.href), // do not change
                    window['pt_ref'] = decodeURI(document.referrer), // do not change
                    window['pt_protocol'] = (("https:" == document.location.protocol) ? "https://" : "http://"), // do not change
                    window['pt_cachebuster'] = new Date().getTime() // do not change
                };
                (function(w,d,s,f,v) {
                    w[v] = w[v] || [];
                ...
                })(window, document,"script","https://utt.pm/utm/268/"++".js","utm268");
            }        
        };
    };
})();
